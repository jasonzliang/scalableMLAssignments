...
void multiply(GNode &node, Galois::UserContext<GNode> &c)
{
  double sum = 0.0;
...
void setYtoX(GNode &node, Galois::UserContext<GNode> &c)
{
...
void multiplyP(GNode &node, Galois::UserContext<GNode> &c)
{
  double sum = 0.0;
  for (auto edge : graph.out_edges(node))
  {
